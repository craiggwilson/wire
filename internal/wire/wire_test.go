...
func loadTestCase(root string, wireGoSrc []byte) (*testCase, error) {
	...
	goFiles := map[string][]byte{
		"github.com/craiggwilson/wire/wire.go": wireGoSrc,
	}
...
func (test *testCase) materialize(gopath string) error {
	...
	// Add go.mod files to example.com and github.com/craiggwilson/wire.
	const importPath = "example.com"
	const depPath = "github.com/craiggwilson/wire"
